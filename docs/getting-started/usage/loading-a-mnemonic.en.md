...
##### Decimal
Enter each word of your BIP39 mnemonic as a number (0-2047) one at a time. You can use [this list](https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt) for reference.

These numbers are zero-based, so the first BIP39 word is `0` and the last is `2047`.

##### Hexadecimal and Octal
You can also enter your BIP39 mnemonic word's numbers (0-2047) in hexadecimal format, with values ranging from 0x0 to 0x7FF, or in octal format, with values ranging from 0 to 3777. This is useful with some metal plate backups that use those formats.
...
Enter the BIP39 mnemonic word's numbers (0-2047) in binary format, toggling the necessary bits to recreate each word's respective index. The last word will have checksum bits dynamically toggled while you fill the bits.

**Tip**: You can use this screen to generate a mnemonic by flipping a coin:

- Flip a coin, if it is heads, mark the first space (value 1) of the word, if it is tails do nothing. Repeat this step for each space up to 1024. If you flip 11 tails in a row, leave the grid blank for 0.
...
Enter the BIP39 mnemonic word's numbers (0-2047) using the Stackbit 1248 metal plate backup method, where each of the four digits of the word's number is a sum of the numbers marked (punched) 1, 2, 4, or 8. For example, to enter the word "oyster", number 1267, you must punch (1)(2)(2,4)(8).
