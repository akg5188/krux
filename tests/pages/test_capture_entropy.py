    ...
    return (
        len(
            [
                ...
                if t("Insufficient entropy!") in call.args[0]
            ]
        )
        > 0
    )
...
def test_cancel_capture(amigo, mocker):
    ...
    # Assert ctx.flash_text was called with the localized cancel message
    from krux.krux_settings import t

    camera_entropy.flash_text.assert_called_with(t("Capture cancelled"))
