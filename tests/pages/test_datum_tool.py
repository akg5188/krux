...
이것은 한국어의 유니코드 줄입니다"""
    ...
    flash_text_args, flash_text_kwargs = ctx.display.flash_text.call_args
    assert flash_text_args[0].startswith(
        "Failed encoding (('Static', (0,))), try as bytes."
    )
    assert flash_text_args[1:] == (248, 2000)
    assert flash_text_kwargs == {"highlight_prefix": ""}
